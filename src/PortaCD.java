public class PortaCD {
    ...
    // int comune = 0;
    // for (int i = 0; i < this.dischi.length; i++) {
    // for (int j = 0; j < p.dischi.length; j++) {
    // if (dischi[i]!=null && p.dischi[j]!=null &&
    // this.dischi[i].equals(p.dischi[j])) {
    // comune++;
    // }
    // }
    // }
    ...
    public int confrontaCollezione(PortaCD p) {
        ...
        int pos = 0;
        int x = 0;
        while (trovato && x <= num_max_cd) {
            int i = 0;
            while (trovato && i <= p.num_max_cd) {
                if (dischi[i] != null && p.getCD(i) != null) {
                    if (dischi[i] != null && p.getCD(i) != null && !(dischi[x].equals(p.dischi[i]))) {
                        trovato=false;
                    }
                }
                i++;
